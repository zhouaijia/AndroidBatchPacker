...
在位于项目的根目录 `build.gradle` 文件中添加Packer Gradle插件的依赖， 如下：

```groovy
buildscript {
    dependencies {
        classpath 'com.aijia.batchpacker:batch-packer:1.0.0'
    }
}
```
...
dependencies {
    compile 'com.aijia.batchpacker:packer_library:1.0.0'
}
```
...
```java
String channel = BatchPackerChannelReader.getChannel(this.getApplicationContext());
```
...
* 支持 productFlavors `./gradlew clean assembleAijiaReleaseChannels`
...
```java
ChannelInfo channelInfo= BatchPackerChannelReader.getChannelInfo(this.getApplicationContext());
if (channelInfo != null) {
   String channel = channelInfo.getChannel();
   Map<String, String> extraInfo = channelInfo.getExtraInfo();
}
// 或者也可以直接根据key获取
String value = BatchPackerChannelReader.get(context, "buildtime");
```
...
- 生成单个渠道包: `./gradlew clean assembleReleaseChannels -PchannelList=aijia`
- 生成多个渠道包: `./gradlew clean assembleReleaseChannels -PchannelList=aijia,dianping`
- 生成渠道包&写入额外信息:  

  `./gradlew clean assembleReleaseChannels -PchannelList=aijia -PextraInfo=buildtime:20161212,hash:xxxxxxx`
...
* [APK Signing Block读取模块: packer_reader](packer_reader/README.md)
* [APK Signing Block写入模块: packer_writer](packer_writer/README.md)
...
* 1.1.3版本起，packer支持对含有comment的apk进行渠道写入
